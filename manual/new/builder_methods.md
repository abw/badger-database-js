...
Multiple arguments are processed in turn, same as calling method
multiple times.

For the sake of brevity these examples show the SQL generated by
the fragment.  They're generally not complete queries and we've
omitted the final `.sql()` method call which is called internally
to generate the SQL.
...
NOTES
// single item string, multi-item string, string with commas
// Object, alias, array.
// Raw sql object, tagged sql
// Call multiple times or pass multiple arguments
...
If you specify a table with an alias then then alias will be used.
...
a table to attach columns to.  The table should previous have been specified
