...
queries on table rows.  These use the [table query builder](table-queries#query-builder)
...
and returns it as a [record](records) object (or array or objects).
...
[records](records) shortly but for now you should know that
it exists.
...
[`insertAll()`](#insertall-array-options), if the value passed is an array,
or `insertOne()` if it's a single data object. You can call these methods
...
[record](records) instead of a row.
...
Read more about records [here](records).
...
[`record()`](#record-row) method to convert it to a [record](records)
object.  Otherwise the row is returned.

You can redefine this method in your own [table classes](table-class)
...
This methods converts a row of data to a [record](records) object.

### records(rows)

This methods converts an array of rows of data to a an array of [record](records) objects.
