...
test( 'multiple columns',
  t => {
    const op = db.build.from('a').columns('b c');
...
test( 'multiple columns with commas',
  t => {
    const op = db.build.from('a').columns('b,c, d');
...
test( 'star',
  t => {
    const op = db.build.from('a').columns('*');
...
test( 'object with columns',
  t => {
    const op = db.build.from('a').columns({ columns: 'id email company.*' });
...
test( 'object with column and alias',
  t => {
    const op = db.build.from('a').columns({ column: 'email', as: 'email_address' });
...
test( 'object with column, table and alias',
  t => {
    const op = db.build.from('a').columns({ table: 'users', column: 'email', as: 'email_address' });
...
test( 'object with table.column and alias',
  t => {
    const op = db.build.from('a').columns({ column: 'users.email', as: 'email_address' });
    t.is( op.sql(), 'SELECT "users"."email" AS "email_address"\nFROM "a"' );
  }
)
...
test( 'object with columns and table',
  t => {
    const op = db.build.from('a').columns({ table: 'users', columns: 'id email' });
...
test( 'object with column and table',
  t => {
    const op = db.build.from('a').columns({ table: 'users', column: 'id email' });
    t.is( op.sql(), 'SELECT "users"."id", "users"."email"\nFROM "a"' );
  }
)
...
test( 'object with columns and prefix',
  t => {
    const op = db.build.from('a').columns({ columns: 'id email', prefix: 'user_' });
...
test( 'object with columns, table and prefix',
  t => {
    const op = db.build.from('a').columns({ table: 'users', columns: 'id email', prefix: "user_" });
...
test( 'two element array',
  t => {
    const op = db.build.from('a').columns(['email', 'email_address']);
    t.is( op.sql(), 'SELECT "a"."email" AS "email_address"\nFROM "a"' );
  }
)
...
test( 'two element array with table',
  t => {
    const op = db.build.from('a').columns(['users.email', 'email_address']);
    t.is( op.sql(), 'SELECT "users"."email" AS "email_address"\nFROM "a"' );
  }
)
...
test( 'columns with table name',
  t => {
    const op = db.build.from('a').columns('x.b c');
...
test( 'sql in object',
  t => {
    const op = db.build.from('a').columns({ sql: 'b as bravo' });
...
test( 'multiple items',
  t => {
    const op = db.build.from('a').columns('b', 'c d', ['e', 'f'], { column: 'x', as: 'y' });
...
test( 'last table in string',
  t => {
    const op = db.build.from('a b').columns('x y');
...
test( 'table with alias',
  t => {
    const op = db.build.from({ table: 'a', as: 'b' }).columns('x y');
    t.is( op.sql(), 'SELECT "b"."x", "b"."y"\nFROM "a" AS "b"' );
  }
)
...
test( 'column with alias from table',
  t => {
    const op = db.build.from({ table: 'a', as: 'b' }).columns({ column: 'x', as: 'y' });
...
test( 'interspersing table() methods',
  t => {
    const op = db.build.from('a d').table('a').columns('b c').table('d').columns('e f');
...
test( 'interspersing table() and prefix() methods',
  t => {
    const op = db.build.from('a d').table('a').prefix('user_').columns('b c').table('d').prefix('company_').columns('e f');
...
test( 'prefix() is ignored when alias is defined',
  t => {
    const op = db.build.from('a').prefix('user_').columns('b', ['c', 'just_c']);
...
test( 'prefix() can be blanked',
  t => {
    const op = db.build.from('a').prefix('user_').columns('b').prefix().columns('c');
