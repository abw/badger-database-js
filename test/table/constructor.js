...
test(
  ...
  t => {
    const t2 = table(mockDatabase, { table: 't2', columns: 'a b' });
...
test(
  ...
  t => {
    const t3 = table(mockDatabase, { table: 't3', id: 'myId', columns: 'myId a b c' });
    t.is( t3.table, 't3' );
    t.is( t3.id, 'myId' );
...
test(
  ...
  t => {
    const t3 = table(mockDatabase, { table: 't3', columns: 'myId:id a b c' });
    t.is( t3.table, 't3' );
    t.is( t3.id, 'myId' );
    t.is( t3.keys.length, 1 );
    t.is( t3.keys[0], 'myId' );
...
test(
  'table with keys',
  t => {
    const t4 = table(mockDatabase, { table: 't4', keys: 'parentId childId', columns: 'parentId childId' });
    t.is( t4.table, 't4' );
...
test(
  ...
  t => {
    const t4 = table(mockDatabase, { table: 't4', columns: 'parentId:key childId:key' });
    t.is( t4.table, 't4' );
    t.is( t4.id, undefined );
    t.is( t4.keys.length, 2 );
    t.is( t4.keys[0], 'parentId' );
